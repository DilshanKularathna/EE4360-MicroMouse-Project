...
// Navigator::exploreAndFindGoal (see Navigator.h for why this is a raw
// function pointer rather than std::function).
bool isBridgeApproachMarker(LineSensorArray &lineSensors) {
    lineSensors.readAll();
    return lineSensors.detectApproachMarker();
}

bool isFinishTile(LineSensorArray &lineSensors) {
    lineSensors.readAll();
...
RobotFSM::RobotFSM(Navigator &navigator, BridgeHandler &bridge, LineSensorArray &lineSensors)
