...
MazePIDController::MazePIDController(float kp, float ki, float kd, float outputMin, float outputMax)
    : _kp(kp), _ki(ki), _kd(kd), _outMin(outputMin), _outMax(outputMax),
      _integral(0.0f), _prevError(0.0f), _firstCall(true) {}

void MazePIDController::setGains(float kp, float ki, float kd) {
    _kp = kp;
    _ki = ki;
    _kd = kd;
}

void MazePIDController::reset() {
    _integral = 0.0f;
    _prevError = 0.0f;
    _firstCall = true;
}

float MazePIDController::compute(float error, float dtSec) {
