...
Navigator::Navigator(MotorDriver &leftMotor, MotorDriver &rightMotor,
...
void Navigator::stopMotors() {
    _leftMotor.stop();
    _rightMotor.stop();
}

void Navigator::turnToHeading(Heading target) {
...
void Navigator::driveWithHeadingHold(float targetHeadingDeg, int16_t baseSpeed, long targetEncoderTicks) {
    _leftEncoder.reset();
    _rightEncoder.reset();
    _headingPid.reset();
    unsigned long lastMicros = micros();
...
void Navigator::driveOneCellForward() {
...
void Navigator::senseWalls(MazeMapper &maze, uint8_t x, uint8_t y, Heading heading) {
...
bool Navigator::exploreAndFindGoal(MazeMapper &maze, uint8_t startX, uint8_t startY, Heading startHeading,
...
void Navigator::runShortestPath(MazeMapper &maze, uint8_t startX, uint8_t startY, Heading startHeading,
